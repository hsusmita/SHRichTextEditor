...
class ImagePickerManager: NSObject {
	...
	func imagePicker(for type: UIImagePickerControllerSourceType) -> UIImagePickerController {
		...
		if type == .camera {
			...
			let overlayView: OverlayView = Bundle.main.loadNibNamed("OverlayView", owner: nil, options: nil)?.first! as! OverlayView
			overlayView.frame = (parentViewController?.view.frame)!
			overlayView.actionOnCancelButtonTap = {
				picker.dismiss(animated: true, completion: { [unowned self] in
					self.actionOnImagePickerDismiss!(nil)
				})
			}
			overlayView.actionOnClickButtonTap = {
				picker.takePicture()
			}
